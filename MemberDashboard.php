        ...
        <div class="container-fluid p-0">
            ...
            <section class="resume-section" id="about">
                <div class="resume-section-content">
                    ...
                    $sql2="SELECT title,description,DAY(date) as day FROM noticeboard WHERE sid='$sid' AND YEAR(date)='$year' AND MONTH(date)='$month' AND DAY(date)='$day'";
                    $result2= mysqli_query($con, $sql2);
                    if(!empty(mysqli_num_rows($result2)))
                    {
                        ...
                        {
                            $txt=$data['title']."  ".$data['description'];
                            $txt= htmlspecialchars($txt);
                            $txt= rawurlencode($txt);
                            $html= file_get_contents("https://translate.google.com/translate_tts?ie=UTF-&&client=gtx&q=".$txt."&tl=en-IN");
                            $player="<audio controls='controls' id='gvoice' autoplay><source src='data:audio/mpeg;base64,".base64_encode($html)."'></audio>";
                    echo $player;
                            echo "<div class='notice'><h2 class='title'><b>".$data['title']."</h2></b><br><h5>".$data['description']."</h5></div>";
                            $i++;
                        }
                    }
                    else
                    {
                        echo "<p class='card-title'>No Notice yet!</p>";
                    }
