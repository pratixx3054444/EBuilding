            ...
            <section class="resume-section" id="about">
                <div class="resume-section-content">
                  ...
                  <?php
                    ...
                    {
                        echo "<h3>SOCIETY REALTED</h3><table class='table'><tr><th>Day<th>Owner<th>Invitation</tr>";
                    ...
                    $sql3="SELECT uid,eid,ename,DAY(date) as day,uevent,status FROM events WHERE sid='$sid' AND category='P' AND YEAR(date)='$year' AND MONTH(date)='$month' ";
                    $result3= mysqli_query($con, $sql3);
                    if(mysqli_num_rows($result3)>0)
                    {
                        echo "<h3>PRIVATE</h3><table class='table'><tr><th>Day<th>Owner<th>Event <th>Invitation<th>Status</tr>";
                        while($data= mysqli_fetch_assoc($result3))
                        {
                            $eid=$data['eid'];
                            $jid=$data['uid'];
                            $qur="SELECT fname from jsociety WHERE jid='$jid'";
                            $data3= mysqli_query($con, $qur);
                            $row2= mysqli_fetch_assoc($data3);
                            
                            echo "<tr><td>".$data['day']."<td>".$row2['fname']."<td>".$data['ename']."<td><img class='img1' src='images//".$data['uevent']."'><td>";
                            if($data['status']=='T')
                            {
                                echo "Accepted</tr>";
                            }
                            else{
                               echo "<a href='acceptEvent.php?eid=".$eid."&jid=".$jid."' class='bn' >ACCEPT</a></tr>";
                            }
                        }
                    }
                    else
                    {
                        die("<p class='card-title'>No Private event!</p>");
                    }
                    ?>
